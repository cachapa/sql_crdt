...
Future<void> main() async {
  ...
  group('Basic', () {
    ...
    test('Create table', () async {
      await crdt.execute('DROP TABLE IF EXISTS test');
      await crdt.execute('''
        CREATE TABLE test (
          id INTEGER NOT NULL,
          name TEXT,
  ...
  group('Watch', () {
    setUp(() async {
      await crdt.execute('DROP TABLE IF EXISTS users');
      await crdt.execute('DROP TABLE IF EXISTS purchases');
    ...
    test('Emit on all selected tables', () async {
      final streamTest = expectLater(
        crdt.watch(
            'SELECT name, price FROM users LEFT JOIN purchases ON users.id = user_id'),
        emitsInOrder([
          [],
          (List<Map<String, Object?>> e) =>
              e.first['name'] == 'John Doe' && e.first['price'] == null,
          (List<Map<String, Object?>> e) =>
              e.first['name'] == 'John Doe' &&
              ((e.first['price'] as double) - 12.3).abs() < 0.01,
        ]),
      );
      await _insertUser(crdt, 1, 'John Doe');
      await _insertPurchase(crdt, 1, 1, 12.3);
